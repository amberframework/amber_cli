...
module AmberCLI::Commands
  class NewCommand < AmberCLI::Core::BaseCommand
    ...
    private def execute_native(full_path_name : String, project_name : String)
      ...
      info "  make setup          # Install shards-alpha + create symlinks"
    ...
    private def execute_web(full_path_name : String, project_name : String)
      ...
      info "To get started:"
      info "  cd #{name}" unless name == "."
      info "  shards-alpha install" if no_deps
      info "  crystal-alpha spec"
    ...
    private def install_dependencies(path : String)
      ...
      warning "Run 'shards-alpha install' in #{path} after resolving the error."
      exit!(error: true)
    rescue ex : File::NotFoundError
      warning "The 'shards-alpha' executable was not found; the project files were kept."
      warning "Install the Amber toolchain and run 'shards-alpha install' in #{path}."
      exit!(error: true)
    end
...
An ECR web application generated by Amber CLI for Amber `2.0.0-beta.5`.
...
The shards-alpha install command creates the checksum-verified lock file
shard.lock. Keep that file in version control with the application.
...
[Amber V2 beta guide](https://github.com/amberframework/amber/blob/v2.0.0-beta.5/docs/beta-installation.md).
README
