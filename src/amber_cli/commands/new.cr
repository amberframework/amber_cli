...
An ECR web application generated by Amber CLI for Amber `2.0.0-beta.3`.
...
[Amber V2 beta guide](https://github.com/amberframework/amber/blob/v2.0.0-beta.3/docs/beta-installation.md).
README
...
<div class="amber-starter">
  <header class="starter-nav">
    <a class="starter-brand" href="/" aria-label="<%= Amber.settings.name %> home">
      <span class="starter-crystal" aria-hidden="true"></span>
...
.amber-starter::after {
  ...
  height: 6rem;
  opacity: 0.17;
  background: linear-gradient(145deg, var(--amber-gold), var(--amber-accent));
  clip-path: polygon(50% 0, 92% 24%, 78% 80%, 50% 100%, 18% 80%, 5% 24%);
...
.starter-crystal {
  ...
  height: 31px;
  flex: 0 0 auto;
  background: linear-gradient(145deg, #ffc36b 0 28%, var(--amber-accent) 29% 68%, var(--amber-accent-deep) 69%);
  clip-path: polygon(50% 0, 93% 24%, 78% 79%, 50% 100%, 18% 79%, 5% 24%);
  filter: drop-shadow(0 4px 6px rgba(169, 61, 8, 0.24));
}

.starter-crystal::after {
  position: absolute;
  inset: 22% 25% 17% 49%;
